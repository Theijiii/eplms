...
export default function FranchisePermitType({ franchise_permit_id }) {
  ...
  return (
    <div className="mx-1 mt-1 p-6 dark:bg-slate-900 bg-white dark:text-slate-300 rounded-lg min-h-screen">
      <h1 className="text-2xl md:text-4xl font-bold mb-8 text-center">Franchise Permit Types</h1>
      ...
      {/* Confirm Modal (temporarily commented out) */}

      <div className="mt-8 text-center">
        <button onClick={() => navigate('/user/dashboard')} className="inline-flex items-center gap-2 text-gray-500 dark:text-slate-400 hover:text-gray-700 dark:hover:text-slate-300 underline">
          <ArrowLeft size={18} />
          Back to Dashboard
        </button>
      </div>
